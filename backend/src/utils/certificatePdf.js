...
// Title bigger and explicitly bold per request (was 16pt) — the banner
// itself grew a little taller to keep the text comfortably inset.
function drawTitleBanner(doc, y, title, marginX = 46) {
  const width = doc.page.width - marginX * 2;
  const bannerH = 34;
  doc.save();
  doc.roundedRect(marginX, y, width, bannerH, 6).fillColor(NAVY).fill();
  doc.lineWidth(1).strokeColor(GOLD).roundedRect(marginX, y, width, bannerH, 6).stroke();
  doc.restore();
  fitCenteredText(doc, sentenceCase(title), marginX, y + 8, width, 19, '#fff', true, 14);
  return y + bannerH + 14;
}
...
async function generateLcPdf({
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // 'Duplicate'), so compare case-insensitively.
      const typeLabel      = sentenceCase(lcType || 'Original');
      const typeLabelColor = typeLabel.toUpperCase() === 'DUPLICATE' ? '#dc2626' : '#1d4ed8';
      doc.save()
      ...
      // Centered under its own line (was left-aligned raw text before, while
      // the Head Master side was already centered under its line — the
      // mismatch read as misaligned).
      doc.font('Helvetica').fontSize(9).fillColor(TEXT)
        .text('Check by / prepared by', 46, LINE_Y + 5, { width: 160, align: 'center', lineBreak: false });
